...
# Use /tmp for downloads, install to /usr/local/bin for global access
DOWNLOAD_DIR="/tmp/claude-downloads"
...
# Run claude install for shell integration (install to /usr/local/bin)
echo "Setting up Claude Code shell integration..."
CLAUDE_INSTALL_DIR="$INSTALL_DIR" "$binary_path" install ${TARGET:+"$TARGET"}
